...
public class TimeSchedule {
    ...
    public TimeSchedule(SessionSettings settings) throws ConfigError,
            FieldConvertError {
    	
        nonStopSession = settings.isSetting(Session.SETTING_NON_STOP_SESSION) && settings.getBool(Session.SETTING_NON_STOP_SESSION);
        TimeZone defaultTimeZone = getDefaultTimeZone(settings);
        if (nonStopSession) {
            startTime = endTime = new TimeEndPoint(NOT_SET, 0, 0, 0, defaultTimeZone);
            return;
        }
        startTime = getTimeEndPoint(settings, defaultTimeZone, Session.SETTING_START_TIME, Session.SETTING_START_DAY);
        endTime = getTimeEndPoint(settings, defaultTimeZone, Session.SETTING_END_TIME, Session.SETTING_END_DAY);
        
    }

    private TimeEndPoint getTimeEndPoint(SessionSettings settings,
            TimeZone defaultTimeZone, String timeSetting, String daySetting) throws ConfigError,
            FieldConvertError {

        Matcher matcher = TIME_PATTERN.matcher(settings.getDefaultProperties().getProperty(timeSetting));
        if (!matcher.find()) {
            throw new ConfigError("could not parse time '"
                    + settings.getDefaultProperties().getProperty(timeSetting) + "'.");
        }

        return new TimeEndPoint(getDay(settings, daySetting, NOT_SET),
                Integer.parseInt(matcher.group(1)), Integer.parseInt(matcher.group(2)),
                Integer.parseInt(matcher.group(3)), getTimeZone(matcher.group(4), defaultTimeZone));
    }

    private TimeZone getDefaultTimeZone(SessionSettings settings)
            throws ConfigError, FieldConvertError {
        TimeZone sessionTimeZone;
        if (settings.isSetting(Session.SETTING_TIMEZONE)) {
            String sessionTimeZoneID = settings.getDefaultProperties().getProperty(Session.SETTING_TIMEZONE);
    ...
    private int getDay(SessionSettings settings, String key, int defaultValue)
            throws ConfigError, FieldConvertError {
        return settings.isSetting(key) ?
                DayConverter.toInteger(settings.getDefaultProperties().getProperty(key))
                : NOT_SET;
    }
